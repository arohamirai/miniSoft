...
char* const WINDOW_MEAN = "mean";
char* const WINDOW_GAUSS = "gauss";
char* const WINDOW_BILATE = "bilate";
...
void setup_windows( )
{
       ...
       cv::namedWindow( WINDOW_MEAN, 0 );
       cv::namedWindow( WINDOW_GAUSS, 0 );
       cv::namedWindow( WINDOW_BILATE, 0 );
...
bool display_next_frame( )
{
	...
	cv::Mat meanMat,gaussMat,bilateMat;
	medianBlur(depth8u,meanMat,5);
	GaussianBlur(depth8u,gaussMat,cvSize(5,5),0,0);
	bilateralFilter(depth8u,bilateMat,5,1,5);
	...
	imshow(WINDOW_MEAN,meanMat);
	imshow(WINDOW_GAUSS,gaussMat);
	imshow(WINDOW_BILATE,bilateMat);
       cvWaitKey( 1 );
